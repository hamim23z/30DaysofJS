...
// The above JSON example is not much different from a normal object. Then, what is the difference ? The difference is THEY KEY OF A JSON OBJECT SHOULD BE WITH DOUBLE QUOTES OR IT SHOULD BE A STRING. //

// Inside the array we have different objects and each objects has keys, each keys has to have double quotes. For instance, we use "firstNaMe" instead of just firstName, however in object we use keys without double quotes. This is the major difference between an object and a JSON. //
...
let usersObj2 = JSON.parse(usersText2, (key, value) => {
    let newValue =
